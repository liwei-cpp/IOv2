 ...
 *
 * @warning 独占访问契约：在一个 istreambuf_iterator 处于活动（尚未走到末尾、仍可能被解引用
 *          或自增）期间，**不得**通过任何其它途径操作其绑定的 streambuf。这包括：另建一个
 *          作用于同一 streambuf 的迭代器、直接调用 `sgetc()`/`sbumpc()`/`sputbackc()`、以及
 *          ——当底层为双向 streambuf 时——通过 `sputc()`/`sputn()` 写入。迭代器自身携带前瞻
 ...
 *
 * @warning Exclusive-access contract: while an istreambuf_iterator is active (not yet at end,
 *          still potentially dereferenced or incremented), the streambuf it is bound to MUST NOT
 *          be operated on through any other path. This includes: constructing another iterator
 *          over the same streambuf, calling `sgetc()`/`sbumpc()`/`sputbackc()` directly, and --
 *          when the underlying streambuf is bidirectional -- writing through
 *          `sputc()`/`sputn()`. The iterator carries look-ahead state; any interleaved stream
 ...
 *
 * @note 与 istreambuf_iterator 不同，本迭代器**不持有任何缓存或前瞻状态**，`operator=(c)`
 *       仅将字符转发给 `streambuf::sputc()`。因此交错的其它流操作不会破坏本迭代器自身的
 ...
 * @tparam TStreamBuf The stream-buffer type; must be a `streambuf` or `ostreambuf`
 *         (i.e. writable).
 * @endif
 */
template <typename TStreamBuf>
