 ...
 *   capable enough for a channel of a given direction.
 * @endif
 */
#pragma once
 ...
 *
 * @note 典型用途是 iochannel/ichannel/ochannel 的 CTAD 推导指引：由「设备 + 工厂」
 *       反推出 iochannel 应采用的字符类型。
 *
 ...
 *
 * @note A typical use is the CTAD deduction guides of iochannel/ichannel/ochannel:
 *       deducing the character type an iochannel should use from a "device + creator" pair.
 *
...
/**
 * @lang{ZH}
 * @brief 转换器 @p TCvt 的读写能力是否够得上方向为 (@p IsIn, @p IsOut) 的通道。
 *
 * 三档判据，与 `base_channel` 的三种实例一一对应：
 * | 方向 | 实例 | 要求 |
 * |---|---|---|
 * | `IsIn && IsOut` | `iochannel` | `support_io_switch`（已蕴含 `support_get` 与 `support_put`） |
 * | 仅 `IsIn` | `ichannel` | `support_get` |
 * | 仅 `IsOut` | `ochannel` | `support_put` |
 *
 ...
 * @tparam IsIn 通道是否具备输入方向。
 * @tparam IsOut 通道是否具备输出方向。
 * @endif
 *
 * @lang{EN}
 * @brief Whether converter @p TCvt is capable enough for a channel whose direction is
 *        (@p IsIn, @p IsOut).
 *
 * Three cases, one per `base_channel` instantiation:
 * | Direction | Instantiation | Requirement |
 * |---|---|---|
 * | `IsIn && IsOut` | `iochannel` | `support_io_switch` (which already implies `support_get` and `support_put`) |
 * | `IsIn` only | `ichannel` | `support_get` |
 * | `IsOut` only | `ochannel` | `support_put` |
 *
 ...
 * @tparam IsIn Whether the channel has an input direction.
 * @tparam IsOut Whether the channel has an output direction.
 * @endif
 */
 ...
 *
 * @note 这是 `base_channel` 两个接收工厂的构造函数所用的判据。不满足时该构造函数被移除，
 ...
 *        capable enough for a channel whose direction is (@p IsIn, @p IsOut).
 *
 * That is, `cvt_direction_capable` applied to the return type of `TCreator::create(TRootCvt)`.
 *
 * @note This is the predicate used by `base_channel`'s two creator-taking constructors. When it
