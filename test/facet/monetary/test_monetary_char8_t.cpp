...
TEST(MonetaryChar8, PutReturnsThePositionAfterTheField)
{
    ...
    std::u8string international(13, u8'^');
    it = obj.put(international.begin() + 2, true, ios, std::u8string(u8"2607"));

    EXPECT_EQ(it, international.begin() + 6);
    EXPECT_EQ(international, u8"^^2607^^^^^^^");
}
...
TEST(MonetaryChar8, PutWritesThroughAnOutputIteratorOntoAStream)
{
    ...
    iochannel chan{mem_device<char8_t>{u8""}};
    obj.put(ochannel_iterator(chan), false, ios, std::u8string(u8"123456"));
    obj.put(ochannel_iterator(chan), true, ios, std::u8string(u8"123456"));
    chan.flush();
    EXPECT_EQ(chan.device().str(), u8"1,234.56123,456");
}
