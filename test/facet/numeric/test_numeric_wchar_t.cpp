...
namespace
{
    ...
    class tunable_conf : public numeric_conf<wchar_t>,
                         public std::enable_shared_from_this<tunable_conf>
    {
    public:
        explicit tunable_conf(const std::string& name = "C")
            : numeric_conf<wchar_t>(name)
            , m_grouping(numeric_conf<wchar_t>::grouping())
            , m_truename(numeric_conf<wchar_t>::truename())
            , m_falsename(numeric_conf<wchar_t>::falsename())
            , m_thousands_sep(numeric_conf<wchar_t>::thousands_sep())
            , m_decimal_point(numeric_conf<wchar_t>::decimal_point())
        {}
        ...
        tunable_conf& yes(std::wstring n)             { m_truename = std::move(n); return *this; }
        tunable_conf& no(std::wstring n)              { m_falsename = std::move(n); return *this; }
    ...
    std::shared_ptr<ctype<wchar_t>> ctype_for(const char* loc)
    {
        return std::make_shared<ctype<wchar_t>>(std::make_shared<ctype_conf<wchar_t>>(loc));
    }

    numeric<wchar_t> facet_for(const char* loc)
    {
        return numeric<wchar_t>(std::make_shared<numeric_conf<wchar_t>>(loc), ctype_for(loc));
    }
    ...
    numeric<wchar_t> facet_of(std::shared_ptr<tunable_conf> conf)
    {
        return numeric<wchar_t>(std::move(conf), ctype_for("C"));
    }

    template <typename TVal>
    std::wstring put_str(const numeric<wchar_t>& obj, ios_base<wchar_t>& io, TVal v)
    {
        std::wstring out;
    ...
    template <typename TVal>
    parse_result<TVal> parse_over_pointers(const numeric<wchar_t>& obj, ios_base<wchar_t>& io,
                                           const std::wstring& input, TVal seed)
    {
    ...
    template <typename TVal>
    parse_result<TVal> parse_over_a_stream(const numeric<wchar_t>& obj, ios_base<wchar_t>& io,
                                           const std::wstring& input, TVal seed)
    {
        parse_result<TVal> res{true, seed, {}};
    ...
    template <typename TVal>
    void expect_parses(const numeric<wchar_t>& obj, ios_base<wchar_t>& io, const std::wstring& input,
                       TVal expected, const std::wstring& rest = L"")
    {
    ...
    template <typename TVal>
    void expect_rejects(const numeric<wchar_t>& obj, ios_base<wchar_t>& io, const std::wstring& input,
                        TVal stored)
    {
        SCOPED_TRACE(::testing::PrintToString(input));
        for (bool streamed : {false, true})
    ...
    std::wstring as_chars(const std::string& ascii) { return std::wstring(ascii.begin(), ascii.end()); }

    // What std::to_chars makes of the same value in the same base: the digits
    // are not the facet's to invent, only to place.
    template <typename TVal>
    std::wstring digits_of(TVal v, int base = 10)
    {
        char       buf[80] = {};
...
TEST(NumericWchar, TheCharacterTypeIsChar)
{
    static_assert(std::is_same_v<numeric<wchar_t>::char_type, wchar_t>);
}

TEST(NumericWchar, ThePunctuationComesFromTheLocale)
{
    const numeric<wchar_t> plain  = facet_for("C");
    const numeric<wchar_t> german = facet_for("de_DE.UTF-8");
...
TEST(NumericWchar, AnUnnamedBooleanFallsBackToAscii)
{
    const numeric_conf<wchar_t> conf("gv_GB.utf8");
    EXPECT_EQ(conf.truename(), L"true");
    EXPECT_EQ(conf.falsename(), L"false");
    EXPECT_EQ(conf.decimal_point(), L'.');
    EXPECT_EQ(conf.thousands_sep(), L',');
}
...
TEST(NumericWchar, ALocaleWithNoSeparatorHasNoGrouping)
{
    const numeric_conf<wchar_t> conf("gl_ES.utf8");
    EXPECT_EQ(conf.decimal_point(), L',');
    EXPECT_TRUE(conf.grouping().empty());
    EXPECT_EQ(conf.thousands_sep(), L'\0');
}

TEST(NumericWchar, ABooleanIsOneOrZeroWithoutBoolalpha)
{
    ios_base<wchar_t>      ios;
    const numeric<wchar_t> obj = facet_for("C");
    EXPECT_EQ(put_str(obj, ios, true), L"1");
    EXPECT_EQ(put_str(obj, ios, false), L"0");
}

TEST(NumericWchar, BoolalphaWritesTheLocaleNames)
{
    const numeric<wchar_t> obj = facet_of(tuned()->yes(L"ja").no(L"nein").ptr());
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::boolalpha);

    EXPECT_EQ(put_str(obj, ios, true), L"ja");
    EXPECT_EQ(put_str(obj, ios, false), L"nein");
}

TEST(NumericWchar, ABooleanNameIsPaddedToTheWidth)
{
    const numeric<wchar_t> obj = facet_of(tuned()->yes(L"ja").ptr());
...
TEST(NumericWchar, AnIntegerIsWrittenAsToCharsWouldWriteIt)
{
    const numeric<wchar_t> obj = facet_for("C");
...
TEST(NumericWchar, ANegativeValueOutsideBaseTenIsItsBitPattern)
{
    const numeric<wchar_t> obj = facet_for("C");
    ...
    ios_base<wchar_t>  decimal;
    const long long least = std::numeric_limits<long long>::min();
    EXPECT_EQ(put_str(obj, decimal, least), digits_of(least, 10));
}

TEST(NumericWchar, ShowbaseWritesTheBasePrefix)
{
    const numeric<wchar_t> obj = facet_for("C");

    ios_base<wchar_t> hex;
    hex.setf(ios_defs::hex, ios_defs::basefield);
    hex.setf(ios_defs::showbase);
    EXPECT_EQ(put_str(obj, hex, 255L), L"0xff");

    // Octal's prefix is a single leading zero, and zero already has one.
    ios_base<wchar_t> octal;
    octal.setf(ios_defs::oct, ios_defs::basefield);
    octal.setf(ios_defs::showbase);
    EXPECT_EQ(put_str(obj, octal, 64L), L"0100");
    EXPECT_EQ(put_str(obj, octal, 0L), L"0");

    // Decimal has no prefix to write.
    ios_base<wchar_t> decimal;
    decimal.setf(ios_defs::showbase);
    EXPECT_EQ(put_str(obj, decimal, 255L), L"255");
}

TEST(NumericWchar, UppercaseAffectsTheHexAlphabetAndItsPrefix)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::hex, ios_defs::basefield);
    ios.setf(ios_defs::showbase | ios_defs::uppercase);
    EXPECT_EQ(put_str(obj, ios, 255L), L"0XFF");
}

TEST(NumericWchar, ShowposWritesAPlusOnANonNegativeDecimal)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::showpos);

    EXPECT_EQ(put_str(obj, ios, 42L), L"+42");
    EXPECT_EQ(put_str(obj, ios, 0L), L"+0");
    EXPECT_EQ(put_str(obj, ios, -42L), L"-42");
}

TEST(NumericWchar, GroupingInsertsTheThousandsSeparator)
{
    ios_base<wchar_t> ios;

    EXPECT_EQ(put_str(facet_of(tuned()->groups({3}).separator(L',').ptr()), ios, 1234567L),
              L"1,234,567");
    EXPECT_EQ(put_str(facet_of(tuned()->groups({1}).separator(L'#').ptr()), ios, 1234L),
              L"1#2#3#4");
    // The last entry of a grouping vector repeats, so {3,2} groups three then
    // twos all the way up.
    EXPECT_EQ(put_str(facet_of(tuned()->groups({3, 2}).separator(L',').ptr()), ios, 12345678L),
              L"1,23,45,678");
    EXPECT_EQ(put_str(facet_of(tuned()->groups({}).separator(L',').ptr()), ios, 1234567L),
              L"1234567");
    ...
    const numeric<wchar_t> grouped = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    EXPECT_EQ(put_str(grouped, ios, -1234567L), L"-1,234,567");

    ios_base<wchar_t> shown;
    shown.setf(ios_defs::showpos);
    EXPECT_EQ(put_str(grouped, shown, 1234567L), L"+1,234,567");
    ...
    ios_base<wchar_t> fixed;
    fixed.setf(ios_defs::fixed, ios_defs::floatfield);
    fixed.precision(3);
    EXPECT_EQ(put_str(grouped, fixed, -1234567.5), L"-1,234,567.500");
}

TEST(NumericWchar, AShortFieldIsPaddedToTheWidth)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ios.fill(L'*');
...
TEST(NumericWchar, InternalPaddingSeparatesTheSignFromTheDigits)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ios.fill(L'*');
    ...
    const std::wstring out = put_str(obj, ios, -42L);
    EXPECT_EQ(out, L"-*****42");
}

// width() is one-shot: the field it sized is the only one it sizes.
TEST(NumericWchar, TheWidthIsConsumedByOnePut)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ios.fill(L'*');
...
TEST(NumericWchar, AFloatIsWrittenAsPrintfWouldWriteIt)
{
    const numeric<wchar_t> obj = facet_for("C");
...
TEST(NumericWchar, ShowpointKeepsTheDecimalPoint)
{
    ...
    EXPECT_EQ(put_str(obj, ios, 1.0), L"1");
    ios.setf(ios_defs::showpoint);
    const std::wstring out = put_str(obj, ios, 1.0);
...
TEST(NumericWchar, TheDecimalPointComesFromTheLocale)
{
    const numeric<wchar_t> obj = facet_of(tuned()->point(L':').ptr());
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::fixed, ios_defs::floatfield);
    ios.precision(2);
    EXPECT_EQ(put_str(obj, ios, 1.5), L"1:50");
}

// Grouping is a property of the integer part; the digits after the point are
// not grouped whatever the locale says.
TEST(NumericWchar, GroupingAppliesToTheIntegerPartOnly)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::fixed, ios_defs::floatfield);
    ios.precision(4);
    EXPECT_EQ(put_str(obj, ios, 1234567.8125), L"1,234,567.8125");
}

TEST(NumericWchar, APointerIsHexadecimalWithItsPrefix)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ...
    int         anchor = 0;
    const void* p      = &anchor;
    const std::wstring out = put_str(obj, ios, p);

    EXPECT_EQ(out.substr(0, 2), L"0x");
    EXPECT_EQ(out, L"0x" + digits_of(reinterpret_cast<std::uintptr_t>(p), 16));
}

// Formatting a pointer needs hex and showbase, but it borrows them: the stream
// is handed back exactly as it was.
TEST(NumericWchar, FormattingAPointerLeavesTheStreamFlagsAlone)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
...
TEST(NumericWchar, WithoutBoolalphaOnlyZeroAndOneAreBooleans)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_parses(obj, ios, L"0", false);
    expect_parses(obj, ios, L"1", true);
    expect_rejects(obj, ios, L"2", true);
}

TEST(NumericWchar, BoolalphaReadsTheLocaleNames)
{
    const numeric<wchar_t> obj = facet_of(tuned()->yes(L"ja").no(L"nein").ptr());
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::boolalpha);

    expect_parses(obj, ios, L"ja", true);
    expect_parses(obj, ios, L"nein", false);
    expect_rejects<bool>(obj, ios, L"vielleicht");
}
...
TEST(NumericWchar, IdenticalBooleanNamesAreAmbiguous)
{
    const numeric<wchar_t> obj = facet_of(tuned()->yes(L"same").no(L"same").ptr());
    ios_base<wchar_t>      ios;
    ios.setf(ios_defs::boolalpha);

    expect_rejects(obj, ios, L"same", false);
}
...
TEST(NumericWchar, WhatPutWritesGetReadsBack)
{
    const numeric<wchar_t> plain   = facet_for("C");
    const numeric<wchar_t> grouped = facet_of(tuned()->groups({3}).separator(L',').ptr());
    ...
    for (const numeric<wchar_t>* obj : {&plain, &grouped})
        for (ios_defs::fmtflags base : bases)
            for (long v : values)
            {
                ...
                const std::wstring field = put_str(*obj, writer, v);
    ...
    auto round_trip = [&](auto v)
    {
        SCOPED_TRACE(::testing::Message() << "value=" << +v);
        ios_base<wchar_t>  writer;
        const std::wstring field = put_str(plain, writer, v);
        ios_base<wchar_t>  reader;
...
TEST(NumericWchar, ALoneSignIsNotANumber)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_rejects<long>(obj, ios, L"-");
    expect_rejects<long>(obj, ios, L"+");
    expect_rejects<double>(obj, ios, L"+");
    expect_rejects<long>(obj, ios, L"");
}
...
TEST(NumericWchar, TheHexPrefixIsReadOnlyWhereHexIsPossible)
{
    const numeric<wchar_t> obj = facet_for("C");

    ios_base<wchar_t> octal;
    octal.setf(ios_defs::oct, ios_defs::basefield);
    expect_parses(obj, octal, L"0x5", 0L, L"x5");

    ios_base<wchar_t> hex;
    hex.setf(ios_defs::hex, ios_defs::basefield);
    expect_parses(obj, hex, L"0x5", 5L);

    // With no base selected the prefix is what selects one.
    ios_base<wchar_t> automatic;
    automatic.setf(static_cast<ios_defs::fmtflags>(0), ios_defs::basefield);
    expect_parses(obj, automatic, L"0x5", 5L);
    expect_parses(obj, automatic, L"010", 8L);
    expect_parses(obj, automatic, L"10", 10L);
}

TEST(NumericWchar, ParsingStopsAtTheFirstForeignCharacter)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_parses(obj, ios, L"42abc", 42L, L"abc");
    expect_parses(obj, ios, L"42 ", 42L, L" ");
    ...
    expect_parses(obj, ios, L"42.5", 42L, L".5");

    const numeric<wchar_t> grouped = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    expect_parses(grouped, ios, L"1,234.5", 1234L, L".5");
}
...
TEST(NumericWchar, AnIntegerOutOfRangeStoresTheExtremeAndFails)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_rejects(obj, ios, std::wstring(40, L'9'), std::numeric_limits<long>::max());
    expect_rejects(obj, ios, L"-" + std::wstring(40, L'9'), std::numeric_limits<long>::min());
}
...
TEST(NumericWchar, AnOversizedDigitGroupIsRejected)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').ptr());
    ios_base<wchar_t>      ios;
    ...
    expect_rejects<long>(obj, ios, std::wstring(256, L'0') + L",5");
    expect_rejects<long>(obj, ios, L"0," + std::wstring(256, L'0'));
}

TEST(NumericWchar, AFloatingPointFieldRoundTrips)
{
    const numeric<wchar_t> obj = facet_for("C");
...
TEST(NumericWchar, ABareZeroIsAFloatingPointNumber)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    expect_parses(obj, ios, L"0", 0.0);
}

TEST(NumericWchar, AnExponentMarkerNeedsItsDigits)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    expect_rejects<double>(obj, ios, L"1e");
}
...
TEST(NumericWchar, AFloatOutOfRangeStoresTheFiniteExtremeAndFails)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_rejects(obj, ios, L"1e400", std::numeric_limits<double>::max());
    expect_rejects(obj, ios, L"-1e400", -std::numeric_limits<double>::max());
}
...
TEST(NumericWchar, AnOversizedGroupIsRejectedAtEveryBoundary)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    ios_base<wchar_t>      ios;
    const std::wstring     big(256, L'1');

    expect_rejects<double>(obj, ios, big + L",5");
    expect_rejects<double>(obj, ios, L"1," + big + L".5");
    expect_rejects<double>(obj, ios, L"1," + big + L"e5");
    expect_rejects<double>(obj, ios, L"1," + big);
}

TEST(NumericWchar, APointerRoundTrips)
{
    const numeric<wchar_t> obj = facet_for("C");
    ...
    ios_base<wchar_t>  writer;
    const std::wstring field = put_str(obj, writer, p);
...
TEST(NumericWchar, PaddingAValueThatStartsWithAZeroIsNotAPrefix)
{
    const numeric<wchar_t> obj = facet_for("C");

    ios_base<wchar_t> ios;
    ios.width(10);
    const std::wstring out = put_str(obj, ios, 0.5);
    EXPECT_EQ(out.size(), 10u);
    EXPECT_EQ(out.substr(out.size() - 3), L"0.5");

    ios_base<wchar_t> internal;
    ...
    internal.fill(L'*');
    internal.width(8);
    EXPECT_EQ(put_str(obj, internal, 255L), L"0x****ff");
}
...
TEST(NumericWchar, AFillThatWouldChangeTheNumberIsRejected)
{
    const numeric<wchar_t> obj = facet_for("C");
    ...
    EXPECT_EQ(put(L'1', ios_defs::internal, 42L), L"");
    EXPECT_EQ(put(L'9', ios_defs::right, 42L), L"");
    // A leading zero reads as the same number.
    EXPECT_EQ(put(L'0', ios_defs::internal, 42L), L"00000042");
    // A '-' in front of a positive number would make it negative.
    EXPECT_EQ(put(L'-', ios_defs::internal, 42L), L"");
    EXPECT_EQ(put(L'-', ios_defs::internal, -42L), L"------42");
    ...
    EXPECT_EQ(put(L'.', ios_defs::internal, 42L), L"");
    EXPECT_EQ(put(L'.', ios_defs::left, 42L), L"42......");
    // Anything that cannot be read into a number is fine.
    EXPECT_EQ(put(L'*', ios_defs::internal, 42L), L"******42");
    EXPECT_EQ(put(L' ', ios_defs::right, 42L), L"      42");
}
...
TEST(NumericWchar, AGroupedFloatAlsoStopsAtTheFirstForeignCharacter)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    ios_base<wchar_t>      ios;

    expect_parses(obj, ios, L"1,234.5abc", 1234.5, L"abc");
    expect_parses(obj, ios, L"1,234.5,6", 1234.5, L",6");
}
...
// positions.
TEST(NumericWchar, AFloatingPointSignIsReadInBothPositions)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_parses(obj, ios, L"+1.5", 1.5);
    expect_parses(obj, ios, L"-1.5", -1.5);
    expect_parses(obj, ios, L"1.5e+3", 1500.0);
    expect_parses(obj, ios, L"1.5e-3", 0.0015);
    expect_parses(obj, ios, L"+1.5E+3", 1500.0);
    expect_parses(obj, ios, L"-1.5E-3", -0.0015);
    expect_parses(obj, ios, L"1.5e3", 1500.0);
}
...
TEST(NumericWchar, PunctuationIsNotReadAsASign)
{
    const numeric<wchar_t> obj = facet_of(tuned()->point(L'-').groups({}).ptr());
    ios_base<wchar_t>      ios;
    expect_parses(obj, ios, L"1-5", 1.5);
}
...
TEST(NumericWchar, OneBooleanNameMayBeAPrefixOfTheOther)
{
    ios_base<wchar_t> ios;
    ios.setf(ios_defs::boolalpha);

    const numeric<wchar_t> prefixed = facet_of(tuned()->yes(L"yes").no(L"y").ptr());
    expect_parses(prefixed, ios, L"yes", true);
    expect_parses(prefixed, ios, L"y", false);

    const numeric<wchar_t> other_way = facet_of(tuned()->yes(L"y").no(L"yes").ptr());
    expect_parses(other_way, ios, L"yes", false);
    expect_parses(other_way, ios, L"y", true);

    const numeric<wchar_t> unnamed = facet_of(tuned()->yes(L"true").no(L"").ptr());
    expect_parses(unnamed, ios, L"true", true);
    expect_rejects<bool>(unnamed, ios, L"");
}
...
TEST(NumericWchar, AGroupedFloatIsGroupedWithOrWithoutAPoint)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());

    ios_base<wchar_t> no_point;
    no_point.setf(ios_defs::fixed, ios_defs::floatfield);
    no_point.precision(0);
    EXPECT_EQ(put_str(obj, no_point, 1234567.0), L"1,234,567");
    EXPECT_EQ(put_str(obj, no_point, -1234567.0), L"-1,234,567");
    ...
    shown.setf(ios_defs::showpos);
    shown.precision(1);
    EXPECT_EQ(put_str(obj, shown, 1234567.5), L"+1,234,567.5");
}
...
TEST(NumericWchar, InfinityAndNotANumberAreNotGrouped)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    ios_base<wchar_t>      ios;

    const std::wstring inf = put_str(obj, ios, std::numeric_limits<double>::infinity());
    const std::wstring nan = put_str(obj, ios, std::numeric_limits<double>::quiet_NaN());
    EXPECT_EQ(inf.find(L','), std::wstring::npos);
    EXPECT_EQ(nan.find(L','), std::wstring::npos);
    EXPECT_EQ(put_str(obj, ios, -std::numeric_limits<double>::infinity()), L"-" + inf);
}
...
// the same number.
TEST(NumericWchar, ALeadingZeroUnderAnExplicitBaseIsJustADigit)
{
    const numeric<wchar_t> obj = facet_for("C");

    ios_base<wchar_t> hex;
    hex.setf(ios_defs::hex, ios_defs::basefield);
    expect_parses(obj, hex, L"0ff", 255L);
    expect_parses(obj, hex, L"00", 0L);

    ios_base<wchar_t> octal;
    octal.setf(ios_defs::oct, ios_defs::basefield);
    expect_parses(obj, octal, L"017", 15L);
}
...
TEST(NumericWchar, AFloatingPointFieldNeedsAtLeastOneDigit)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;

    expect_rejects<double>(obj, ios, L".");
    expect_rejects<double>(obj, ios, L"e5");
    expect_rejects<double>(obj, ios, L".e5");

    // Past the first point or exponent, a second one ends the field rather than
    // extending it.
    expect_parses(obj, ios, L"1.5.5", 1.5, L".5");
    expect_parses(obj, ios, L"1e5e5", 100000.0, L"e5");
    expect_parses(obj, ios, L"1.5e5.5", 150000.0, L".5");
}
...
TEST(NumericWchar, AnIntegerSignIsNotReadWhenItIsPunctuation)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L'-').point(L'.').ptr());
    ios_base<wchar_t>      ios;
    expect_parses(obj, ios, L"1-234", 1234L);
}
...
TEST(NumericWchar, AFloatIsGroupCheckedAtEveryBoundary)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L',').point(L'.').ptr());
    ios_base<wchar_t>      ios;

    expect_parses(obj, ios, L"1,234", 1234.0);
    expect_parses(obj, ios, L"1,234.5", 1234.5);
    expect_parses(obj, ios, L"1,234e2", 123400.0);
    expect_rejects<double>(obj, ios, L"12,34");
    expect_rejects<double>(obj, ios, L"12,34.5");
    expect_rejects<double>(obj, ios, L"12,34e2");
}
...
TEST(NumericWchar, AnEmptyBooleanNameMatchesNothing)
{
    ios_base<wchar_t> ios;
    ios.setf(ios_defs::boolalpha);

    const numeric<wchar_t> no_true = facet_of(tuned()->yes(L"").no(L"false").ptr());
    expect_parses(no_true, ios, L"false", false);
    expect_rejects<bool>(no_true, ios, L"true");
}
...
TEST(NumericWchar, EveryFloatingPointWidthRefusesTheSameNonNumbers)
{
    const numeric<wchar_t> obj = facet_for("C");
    ios_base<wchar_t>      ios;
    ...
    auto refuses = [&](auto tag)
    {
        ...
        const std::wstring not_numbers[] = {L".", L"e5", L".e5", L"+", L"-", L"", L"1e"};
        for (const std::wstring& input : not_numbers)
        {
            SCOPED_TRACE(::testing::PrintToString(input));
        ...
        expect_parses(obj, ios, L"1", TVal{1});
        expect_parses(obj, ios, L"1.5", TVal{1.5});
        expect_parses(obj, ios, L"1e2", TVal{100});
        expect_parses(obj, ios, L"1.5e2", TVal{150});
        expect_parses(obj, ios, L"1.5.5", TVal{1.5}, L".5");
        expect_parses(obj, ios, L"1e5e5", TVal{100000}, L"e5");
    };
...
// number at all.
TEST(NumericWchar, ALeadingPunctuationSignIsNotASign)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3}).separator(L'-').point(L'+').ptr());
    ios_base<wchar_t>      ios;

    expect_rejects<long>(obj, ios, L"-234");
    expect_rejects<double>(obj, ios, L"-234");
...
TEST(NumericWchar, AZeroEndsTheGroupingRule)
{
    const numeric<wchar_t> obj = facet_of(tuned()->groups({3, 0}).separator(L',').ptr());
    ios_base<wchar_t>      ios;

    EXPECT_EQ(put_str(obj, ios, 1234567L), L"1234,567");
    expect_parses(obj, ios, L"1234,567", 1234567L);
    expect_parses(obj, ios, L"1,567", 1567L);
    expect_rejects<long>(obj, ios, L"1234,56");
}
