...
void test_collate_wchar_t_compare_1()
{
    dump_info("Test collate<wchar_t> compare 1...");
    const IOv2::collate obj(std::make_shared<IOv2::collate_conf<wchar_t>>("C"));
    
    wchar_t strlit1[] = L"monkey picked tikuanyin oolong";
    wchar_t strlit2[] = L"imperial tea court green oolong";
    ...
    { // compare with const char & list as input
        std::list<wchar_t> lstLit1{strlit1, strlit1 + 7};
        std::list<wchar_t> lstLit2{strlit2, strlit2 + size2};
    ...
    { // compare with const list as input
        std::list<wchar_t> lstLit1{strlit1, strlit1 + size1};
...
void test_collate_wchar_t_compare_2()
{
    dump_info("Test collate<wchar_t> compare 2...");
    const IOv2::collate obj(std::make_shared<IOv2::collate_conf<wchar_t>>("de_DE.UTF-8"));
    ...
    {  // compare with const char & list as input
        std::list<wchar_t> lstLit1{strlit3, strlit3 + 7};
        std::list<wchar_t> lstLit2{strlit4, strlit4 + size4};
    ...
    {  // compare with list as input
        std::list<wchar_t> lstLit1{strlit3, strlit3 + size3};
        std::list<wchar_t> lstLit2{strlit4, strlit4 + size4};
...
void test_collate_wchar_t_compare_3()
{
    dump_info("Test collate<wchar_t> compare 3...");
    const IOv2::collate obj_c(std::make_shared<IOv2::collate_conf<wchar_t>>("C"));
    const IOv2::collate obj_de(std::make_shared<IOv2::collate_conf<wchar_t>>("de_DE.UTF-8"));
    ...
    {  // compare with const char as input
        ...
        VERIFY(obj_de.compare(strlit3, strlit3 + 3, strlit4, strlit4 + 3) == std::strong_ordering::less);
        VERIFY(obj_c.compare(strlit1, strlit1 + 3, strlit1, strlit1 + 4) == std::strong_ordering::less);
        VERIFY(obj_de.compare(strlit3, strlit3 + 4, strlit3, strlit3 + 3) == std::strong_ordering::greater);
        VERIFY(obj_c.compare(strlit1, strlit1 + 4, strlit4, strlit4 + 1) == std::strong_ordering::greater);
        VERIFY(obj_de.compare(strlit3, strlit3 + 3, strlit3, strlit3 + 3) == std::strong_ordering::equal);
    ...
    {  // compare with deque as input
        std::deque<wchar_t> deqlit1{std::begin(strlit1), std::end(strlit1)};
        std::deque<wchar_t> deqlit2{std::begin(strlit2), std::end(strlit2)};
        std::deque<wchar_t> deqlit3{std::begin(strlit3), std::end(strlit3)};
        std::deque<wchar_t> deqlit4{std::begin(strlit4), std::end(strlit4)};
        std::deque<wchar_t> deqlit5{std::begin(strlit5), std::end(strlit5)};
        std::deque<wchar_t> deqlit6{std::begin(strlit6), std::end(strlit6)};
        ...
        VERIFY(obj_de.compare(deqlit3.begin(), deqlit3.begin() + 3, deqlit4.begin(), deqlit4.begin() + 3) == std::strong_ordering::less);
        VERIFY(obj_c.compare(deqlit1.begin(), deqlit1.begin() + 3, deqlit1.begin(), deqlit1.begin() + 4) == std::strong_ordering::less);
        VERIFY(obj_de.compare(deqlit3.begin(), deqlit3.begin() + 4, deqlit3.begin(), deqlit3.begin() + 3) == std::strong_ordering::greater);
        VERIFY(obj_c.compare(deqlit1.begin(), deqlit1.begin() + 4, deqlit4.begin(), deqlit4.begin() + 1) == std::strong_ordering::greater);
        VERIFY(obj_de.compare(deqlit3.begin(), deqlit3.begin() + 3, deqlit3.begin(), deqlit3.begin() + 3) == std::strong_ordering::equal);
    ...
    {  // compare with const char & deque as input
        ...
        VERIFY(obj_c.compare(strlit3, strlit3 + 3, deqlit4.begin(), deqlit4.begin() + 3) == std::strong_ordering::greater);
        VERIFY(obj_de.compare(deqlit3.begin(), deqlit3.begin() + 3, strlit4, strlit4 + 3) == std::strong_ordering::less);
        VERIFY(obj_de.compare(strlit3, strlit3 + 3, deqlit4.begin(), deqlit4.begin() + 3) == std::strong_ordering::less);
        ...
        VERIFY(obj_de.compare(deqlit3.begin(), deqlit3.begin() + 4, strlit3, strlit3 + 3) == std::strong_ordering::greater);
        VERIFY(obj_de.compare(strlit3, strlit3 + 4, deqlit3.begin(), deqlit3.begin() + 3) == std::strong_ordering::greater);
        ...
        VERIFY(obj_de.compare(deqlit3.begin(), deqlit3.begin() + 3, strlit3, strlit3 + 3) == std::strong_ordering::equal);
        VERIFY(obj_de.compare(strlit3, strlit3 + 3, deqlit3.begin(), deqlit3.begin() + 3) == std::strong_ordering::equal);
...
void test_collate_wchar_t_transform_1()
{
    dump_info("Test collate<wchar_t> transform 1...");
    const IOv2::collate obj(std::make_shared<IOv2::collate_conf<wchar_t>>("de_DE.UTF-8"));
    ...
    {  // transform_length with list iterator as input
        std::list<wchar_t> s1{std::begin(strlit3), std::end(strlit3)};
        std::list<wchar_t> s2{std::begin(strlit4), std::end(strlit4)};
        
        VERIFY(obj.transform_length(s1.begin(), s1.end()) == len1);
        VERIFY(obj.transform_length(s2.begin(), s2.end()) == len2);
    }
    
    std::wstring str_trans1, str_trans2;
    ...
    {   // transform using list as input and char array as output
        std::list<wchar_t> lstlit3{std::begin(strlit3), std::end(strlit3)};
        std::list<wchar_t> lstlit4{std::begin(strlit4), std::end(strlit4)};
        std::wstring str_trans2_1, str_trans2_2;
...
void test_collate_wchar_t_transform_2()
{
    dump_info("Test collate<wchar_t> transform 2...");
    const IOv2::collate obj(std::make_shared<IOv2::collate_conf<wchar_t>>("C"));
    
    constexpr size_t MAX_SIZE = 10000000;
    const std::wstring sstr(MAX_SIZE, L'a');
    ...
    {   // transform using list as input and char array as output
        std::list<wchar_t> lstlit3{sstr.begin(), sstr.end()};
        std::wstring str_trans2_1; str_trans2_1.resize(len1);
        auto [it, s] = obj.transform(lstlit3.begin(), lstlit3.end(), str_trans2_1.data());
        VERIFY(s == len1);
        VERIFY(it == str_trans2_1.data() + len1);
        VERIFY(str_trans2_1 == str_trans1);
...
void test_collate_wchar_t_transform_3()
{
    dump_info("Test collate<wchar_t> transform 3...");
    const IOv2::collate obj_c(std::make_shared<IOv2::collate_conf<wchar_t>>("C"));    
    const IOv2::collate obj_de(std::make_shared<IOv2::collate_conf<wchar_t>>("de_DE.UTF-8"));
    ...
    {  // transform_length with list iterator as input
        std::list<wchar_t> s1{std::begin(strlit1), std::end(strlit1)};
        std::list<wchar_t> s2{std::begin(strlit2), std::end(strlit2)};
        std::list<wchar_t> s3{std::begin(strlit3), std::end(strlit3)};
        std::list<wchar_t> s4{std::begin(strlit4), std::end(strlit4)};
        std::list<wchar_t> s5{std::begin(strlit5), std::end(strlit5)};
        std::list<wchar_t> s6{std::begin(strlit6), std::end(strlit6)};
        
        VERIFY(obj_c.transform_length(s1.begin(), s1.end()) == len1_c);
        VERIFY(obj_c.transform_length(s2.begin(), s2.end()) == len2_c);
        VERIFY(obj_c.transform_length(s3.begin(), s3.end()) == len3_c);
        ...
        VERIFY(obj_de.transform_length(s3.begin(), s3.end()) == len3_de);
...
void test_collate_wchar_t_transform_4()
{
    dump_info("Test collate<wchar_t> transform 4...");
    const IOv2::collate obj(std::make_shared<IOv2::collate_conf<wchar_t>>("de_DE.UTF-8"));
    
    const wchar_t strlit[] = L"Äuglein Augment\0Base baß Baß Bast";
    const wchar_t strlit1[] = L"Äuglein Augment";
    const wchar_t strlit2[] = L"Base baß Baß Bast";
    ...
    
    {
        ...
        std::list<wchar_t> ls_strlit(std::begin(strlit), std::end(strlit));
        std::wstring t3, t4;
        t3.resize(len1 - 3); t4.resize(len - 3);
