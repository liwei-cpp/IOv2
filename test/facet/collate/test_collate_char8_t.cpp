...
void test_collate_char8_t_transform_2()
{
    ...
    {   // transform using char array as input / output
        ...
        if (s > len1) throw std::runtime_error("collate::transform fail.");
        if (it != str_trans1.data() + s) throw std::runtime_error("collate::transform fail.");
        str_trans1.resize(s);
    ...
    {   // transform using list as input and char array as output
        ...
        auto [it, s] = obj.transform(lstlit3.begin(), lstlit3.end(), str_trans2_1.data());
        if (s > len1) throw std::runtime_error("collate::transform fail.");
        if (it != str_trans2_1.data() + s) throw std::runtime_error("collate::transform fail.");
        str_trans2_1.resize(s);
        if (str_trans2_1 != str_trans1) throw std::runtime_error("collate::transform fail.");
    }
...
void test_collate_char8_t_transform_4()
{
...

    {
        ...
        if (t3.size() != t1.size() - 3) throw std::runtime_error("collate::transform fail.");
