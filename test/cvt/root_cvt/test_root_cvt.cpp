void test_root_cvt_mem_gen_1();
void test_root_cvt_mem_gen_2();
void test_root_cvt_mem_gen_3();
void test_root_cvt_mem_get_1();
void test_root_cvt_mem_get_nra_1();
void test_root_cvt_mem_put_1();
void test_root_cvt_mem_seek_1();
void test_root_cvt_mem_seek_2();
void test_root_cvt_mem_reset_1();
void test_root_cvt_mem_device_1();
void test_root_cvt_mem_detach_1();
void test_root_cvt_mem_detach_2();
void test_root_cvt_mem_attach_1();
void test_root_cvt_mem_attach_2();
void test_root_cvt_mem_self_assignment();

void test_root_cvt_std_gen_1();
void test_root_cvt_std_gen_2();
void test_root_cvt_std_gen_3();
void test_root_cvt_std_get_1();
void test_root_cvt_std_get_nra_1();
void test_root_cvt_std_put_1();

void test_root_cvt_file_gen_1();
void test_root_cvt_file_gen_2();
void test_root_cvt_file_gen_3();
void test_root_cvt_file_get_1();
void test_root_cvt_file_get_nra_1();
void test_root_cvt_file_put_1();
void test_root_cvt_file_seek_1();
void test_root_cvt_file_seek_2();
void test_root_cvt_file_reset_1();
void test_root_cvt_file_device_1();
void test_root_cvt_file_detach_1();
void test_root_cvt_file_detach_2();
void test_root_cvt_file_attach_1();
void test_root_cvt_file_attach_2();

void test_root_cvt()
{
    test_root_cvt_mem_gen_1();
    test_root_cvt_mem_gen_2();
    test_root_cvt_mem_gen_3();
    test_root_cvt_mem_get_1();
    test_root_cvt_mem_get_nra_1();
    test_root_cvt_mem_put_1();
    test_root_cvt_mem_seek_1();
    test_root_cvt_mem_seek_2();
    test_root_cvt_mem_reset_1();
    test_root_cvt_mem_device_1();
    test_root_cvt_mem_detach_1();
    test_root_cvt_mem_detach_2();
    test_root_cvt_mem_attach_1();
    test_root_cvt_mem_attach_2();
    test_root_cvt_mem_self_assignment();
    
    test_root_cvt_std_gen_1();
    test_root_cvt_std_gen_2();
    test_root_cvt_std_gen_3();
    test_root_cvt_std_get_1();
    test_root_cvt_std_get_nra_1();
    test_root_cvt_std_put_1();
    
    test_root_cvt_file_gen_1();
    test_root_cvt_file_gen_2();
    test_root_cvt_file_gen_3();
    test_root_cvt_file_get_1();
    test_root_cvt_file_get_nra_1();
    test_root_cvt_file_put_1();
    test_root_cvt_file_seek_1();
    test_root_cvt_file_seek_2();
    test_root_cvt_file_reset_1();
    test_root_cvt_file_device_1();
    test_root_cvt_file_detach_1();
    test_root_cvt_file_detach_2();
    test_root_cvt_file_attach_1();
    test_root_cvt_file_attach_2();
}