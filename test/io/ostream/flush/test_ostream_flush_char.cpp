...
static_assert(std::is_copy_constructible_v<decltype(ostream(mem_device<char>{}))>,
              "IOv2::ostream<mem_device<char>> must remain copy-constructible");
static_assert(std::is_copy_constructible_v<decltype(iostream(mem_device<char>{}))>,
              "IOv2::iostream<mem_device<char>> must remain copy-constructible");

TEST(OstreamFlushChar, FlushingAStreamWithNothingPendingLeavesTheDeviceAlone)
{
    auto helper = []<template <typename, typename> class T>()
    {
        const std::string seeded("already there");

        T with(mem_device{seeded});
        T without(mem_device{""});
...
TEST(OstreamFlushChar, FlushIsIdempotent)
{
    auto helper = []<template <typename, typename> class T>()
    {
        T os(mem_device{""});
        ...
        const std::string after_first = os.device().str();
        os.flush();
        os.flush();
...
TEST(OstreamFlushChar, ATieCycleIsRejectedWhenTheTieIsSet)
{
    auto helper = []<template <typename, typename> class T>()
    {
        T a(mem_device{""});
...
TEST(OstreamFlushChar, ADeviceThatRefusesToFlushIsReportedAsDevfailbit)
{
    auto helper = []<template <typename, typename> class T>()
    {
...
TEST(OstreamFlushChar, ConcurrentFlushesDoNotCorruptTheStream)
{
    auto helper = []<template <typename, typename> class T>()
    {
