...
/**
 * istream<char>::tell(): reporting the read position.
 *
...
TEST(IstreamTellChar, TellStartsAtZeroAndCountsWhatWasConsumed)
{
...
TEST(IstreamTellChar, TellNamesWhereTheNextReadWillStart)
{
...
TEST(IstreamTellChar, TellDoesNotConsumeAndRepeatsItsAnswer)
{
    auto expect_idempotent = []<template <typename, typename> class T>()
    {
        ...
        EXPECT_EQ(is.tell(), first);
        EXPECT_EQ(is.get(), '6');       // still where it was
    };
...
TEST(IstreamTellChar, AStreamWithNothingInItIsStillAtPositionZero)
{
...
TEST(IstreamTellChar, AFileAndAMemoryStreamCountAlike)
{
...
TEST(IstreamTellChar, AnUnreportablePositionIsEmptyRatherThanWrong)
{
