void test_istream_ws_char_1();
void test_istream_function_manip_char_1();
void test_istream_null_manip_char_1();
void test_istream_ws_no_ctype_char_1();
void test_istream_tied_flush_char_1();

void test_istream_ws_wchar_t_1();
void test_istream_function_manip_wchar_t_1();
void test_istream_null_manip_wchar_t_1();
void test_istream_ws_no_ctype_wchar_t_1();
void test_istream_tied_flush_wchar_t_1();

void test_istream_ws()
{
    test_istream_ws_char_1();
    test_istream_function_manip_char_1();
    test_istream_null_manip_char_1();
    test_istream_ws_no_ctype_char_1();
    test_istream_tied_flush_char_1();

    test_istream_ws_wchar_t_1();
    test_istream_function_manip_wchar_t_1();
    test_istream_null_manip_wchar_t_1();
    test_istream_ws_no_ctype_wchar_t_1();
    test_istream_tied_flush_wchar_t_1();
}
