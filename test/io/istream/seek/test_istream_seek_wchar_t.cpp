...
/**
 * The same seek() contract as test_istream_seek_char.cpp for wchar_t.
 *
...
TEST(IstreamSeekWchar, SeekSendsTheNextReadWhereItWasAsked)
{
    auto expect_moved = []<template <typename, typename> class T>()
    {
        ...
        is.seek(10);
        EXPECT_EQ(is.get(), L'a');     // index 10

        is.seek(0);
        EXPECT_EQ(is.get(), L'0');

        is.seek(15);
        EXPECT_EQ(is.get(), L'f');
    };
...
TEST(IstreamSeekWchar, SeekingRelativeToTheCurrentPositionAdvancesByThatMuch)
{
...
TEST(IstreamSeekWchar, SeekClearsEndOfFile)
{
...
TEST(IstreamSeekWchar, ASuccessfulSeekLeavesTheStateOtherwiseAlone)
{
...
TEST(IstreamSeekWchar, SeekingPastTheEndFailsAndLeavesNoPositionToReport)
{
...
TEST(IstreamSeekWchar, SeekWorksOverAFileDevice)
{
